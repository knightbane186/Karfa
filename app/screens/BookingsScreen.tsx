...
const BookingsScreen = () => {
  ...
  const loadBookmarks = async () => {
    try {
      ...
      if (storedBookmarks) {
        ...
        setIsBookmarked(bookmarks.has(Number(id)));
      }
    } catch (error) {
  ...
  const handleBookmarkToggle = async () => {
    try {
      const storedBookmarks = await AsyncStorage.getItem('businessBookmarks');
      let bookmarks = new Set(storedBookmarks ? JSON.parse(storedBookmarks) : []);
      ...
      await AsyncStorage.setItem('businessBookmarks', JSON.stringify(Array.from(bookmarks)));
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={handleBookmarkToggle} style={styles.bookmarkButton}>
          <Ionicons 
            ...
            size={24} 
            color={isBookmarked ? "#FF0000" : "white"} 
          />
        </TouchableOpacity>
      </View>
...
//     setIsBookmarked(!isBookmarked);
//     // Here you would typically also update this in your global state or storage
//   };
